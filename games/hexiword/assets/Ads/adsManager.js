...
(function (global) {
  ...
  function adsSystemOn() {
    return !!(cfg && cfg.enabled && adsEnabled);
  }

  function applyProviderEnabled() {
    if (provider && typeof provider.setEnabled === "function") {
      provider.setEnabled(adsSystemOn());
    }
  }
  ...
  function isWarnMessage(msg) {
    return /blocked|failed|threw|not-available|placement_unknown|placement_disabled|ads_disabled|no_provider|already_showing/i.test(
      msg
    );
  }

  function log() {
    if (!cfg || !cfg.debugLogs) return;
    var args = Array.prototype.slice.call(arguments);
    var msg = args.map(formatLogArg).join(" ");
    if (global.console && typeof global.console.log === "function") {
      var cargs = args.slice();
      cargs.unshift("[Ads]");
      global.console.log.apply(global.console, cargs);
    }
    if (global.AdsOverlay && typeof global.AdsOverlay.pushLog === "function") {
      global.AdsOverlay.pushLog(isWarnMessage(msg) ? "warn" : "log", msg);
    }
  }
  ...
  function evaluateInterstitial(placement, mutate) {
    ...
    // Debug-menu taps skip local frequency / enabled so FakeAds can be
    // exercised while the game is otherwise ads-off. KWAds still will not
    // call the host SDK when the ads system is disabled.
    if (isDebugPlacement(placement)) return { detail: null, counter: counter };

    if (!adsSystemOn()) return { detail: DETAIL.ADS_DISABLED, counter: counter };
  ...
  /** True when the host injected at least one real kw* ad function. */
  function hostAdsApiPresent() {
    return (
      typeof global.kwRequestInterstitial === "function" ||
      typeof global.kwRequestRewardedAd === "function" ||
      typeof global.kwShowBanner === "function"
    );
  }

  /**
   * Honour config.provider. When the host API is present, KWAds is exclusive —
   * no FakeAds fallback. Offline / missing function / no fill is not-available.
   * FakeAds is only for preview, when no kw* function exists at all.
   */
  function resolveProvider() {
    ...
    if (want === "kwads") return hasKW ? new global.KWAdsProvider() : null;

    if (hasKW && hostAdsApiPresent()) return new global.KWAdsProvider();
    if (hasFake) return new global.FakeAdsProvider();
    if (hasKW) return new global.KWAdsProvider();
    return null;
  }
  ...
  function showRewardedAd(placement, cb) {
    ...
    if (!isDebugPlacement(placement) && !adsSystemOn()) {
      log("rewarded blocked:", DETAIL.ADS_DISABLED, placement);
      return Promise.resolve(settle(blocked(DETAIL.ADS_DISABLED, placement, { earned: false })));
    }
  ...
  function showBanner(context) {
    ...
    var debugBanner = isDebugPlacement(context);
    if (
      !debugBanner &&
      (!adsSystemOn() || !(cfg.banner && cfg.banner.enabled))
    ) {
  ...
  /**
   * Kill switch. False = do not load any ads (interstitial, rewarded, banner).
   * KWAds is told to stop requesting from the host SDK as well.
   * @param {boolean} enabled
   */
  function setEnabled(enabled) {
    adsEnabled = !!enabled;
    applyProviderEnabled();
    if (!adsSystemOn()) destroyBanner();
  ...
  /**
   * Show or hide the on-screen Ads log HUD. Off by default. Requires debugLogs.
   * @param {boolean} on
   */
  function setLogHudEnabled(on) {
    if (global.AdsOverlay && typeof global.AdsOverlay.setLogHudEnabled === "function") {
      global.AdsOverlay.setLogHudEnabled(on);
    }
  }
  ...
  function getLogHudEnabled() {
    return !!(
      global.AdsOverlay &&
      typeof global.AdsOverlay.getLogHudEnabled === "function" &&
      global.AdsOverlay.getLogHudEnabled()
    );
  }
  ...
  function getInterstitialGate() {
    ...
    if (!cfg || !adsSystemOn()) detail = DETAIL.ADS_DISABLED;
